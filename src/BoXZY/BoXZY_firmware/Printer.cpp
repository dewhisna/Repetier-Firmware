...
bool Printer::has_L;
uint16_t Printer::L_index;
uint16_t Printer::L_end_index;
bool Printer::is_L_in_focus_mode;
...
#endif
BoXZY_head_t Printer::BoXZY_head = BoXZY_unknown_head;
#ifdef DEBUG_PRINT
int debugWaitLoop = 0;
#endif
...
uint8_t Printer::setDestinationStepsFromGCode(GCode *com)
{
    ...
    bool is_move = !com->hasNoXYZ()
        || (com->hasE() && destinationSteps[E_AXIS] != currentPositionSteps[E_AXIS]);

    Printer::has_L = com->hasL();
    if (Printer::has_L)
    {
        if (!is_move)
        {
            BoXZYLBuffer.free_recently_committed(com->L_index, com->L_end_index);
        }
        else
        {
            Printer::L_index = com->L_index;
            Printer::L_end_index = com->L_end_index;
        }
    }

    return is_move;  // ignore unproductive moves
}
