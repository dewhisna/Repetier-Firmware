...
void PrintLine::queueCartesianMove(uint8_t check_endstops, uint8_t pathOptimize)
{
    ...
    p->has_L = Printer::has_L;
    Printer::has_L = false;
    p->L_index = Printer::L_index;
    p->L_end_index = Printer::L_end_index;
    ...
    p->delta[L_AXIS] = !p->has_L
        ? 0
        : BoXZYLBuffer.subtract(p->L_end_index, p->L_index);

    if(p->isNoMove())
    {
//        OUT_P_I_LN("c=", PrintLine::linesCount);
        if(newPath)   // need to delete dummy elements, otherwise commands can get locked.
            resetPathPlanner();
        //OUT_P_I_LN("c=", PrintLine::linesCount);
        if (p->has_L)
        {
            BoXZYLBuffer.free_recently_committed(p->L_index, p->L_end_index);
        }
        return; // No steps included
    }
    float xydist2;
    ...
    //Define variables that are needed for the Bresenham algorithm. Please note that  Z is not currently included in the Bresenham algorithm.
    // L_AXIS can never be the primaryAxis.
    if(p->delta[Y_AXIS] > p->delta[X_AXIS] && p->delta[Y_AXIS] > p->delta[Z_AXIS] && p->delta[Y_AXIS] > p->delta[E_AXIS]) p->primaryAxis = Y_AXIS;
    else if (p->delta[X_AXIS] > p->delta[Z_AXIS] && p->delta[X_AXIS] > p->delta[E_AXIS]) p->primaryAxis = X_AXIS;
    else if (p->delta[Z_AXIS] > p->delta[E_AXIS]) p->primaryAxis = Z_AXIS;
    else p->primaryAxis = E_AXIS;
...
void PrintLine::calculateMove(float axisDistanceMM[], uint8_t pathOptimize,fast8_t drivingAxis)
{
    ...
    error[L_AXIS] = 0; // Turn laser on immediately
#endif
#if NONLINEAR_SYSTEM
...
int32_t PrintLine::bresenhamStep() // Version for delta printer
{
...
#endif
    {
        ...
        if (cur->numNonlinearSegments)
        {
            ...
            cur->error[L_AXIS] = 0; // Turn laser on immediately
...
//        OUT_P_I_LN("SR", cur->stepsRemaining);
        HAL::allowInterrupts();
        lastblk = -1;
#if INCLUDE_DEBUG_NO_MOVE
        if(Printer::debugNoMoves())   // simulate a move, but do nothing in reality
    ...
    for(fast8_t loop = 0; loop < max_loops; loop++)
    {
        ...
        Printer::executeXZGantrySteps();
            ...
            if (cur->has_L)
            {
                if((cur->error[L_AXIS] -= cur->delta[L_AXIS]) < 0)
                {
                    uint8_t power = BoXZYLBuffer.pop();
                    set_laser(power);
                    cur->has_L = (BoXZYLBuffer.oldest_index != cur->L_end_index);
                    cur->error[L_AXIS] += cur_errupd;
                }
            }
    ...
    if(cur->stepsRemaining <= 0 || cur->isNoMove())   // line finished
    {
        ...
        if (Printer::BoXZY_head == BoXZY_Laser_head)
        {
            if (cur->has_L)
            {
                while (BoXZYLBuffer.oldest_index != cur->L_end_index
                        && BoXZYLBuffer.oldest_index != BoXZYLBuffer.unclaimed_index)
                {
                    BoXZYLBuffer.pop();
                }
                cur->has_L = false;
