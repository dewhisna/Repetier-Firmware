...
void Commands::setFanSpeed(int speed, bool immediately) {
    ...
    if (Printer::BoXZY_head == BoXZY_3D_Printer_head)
    {
        ...
        if(PrintLine::linesCount == 0 || immediately) {
            ...
            Printer::setFanSpeedDirectly(speed); // Ignored by the HAL when using BoXZY_Laser_head
        }
...
void Commands::processArc(GCode *com) {
    ...
    if(!Printer::setDestinationStepsFromGCode(com)) return; // For X Y Z E F L
    ...
    switch(com->G) {
        ...
        case 1: // G1
#if defined(SUPPORT_LASER) && SUPPORT_LASER
            {
                ...
                if(Printer::setDestinationStepsFromGCode(com)) // For X Y Z E F L
#if NONLINEAR_SYSTEM
    ...
    switch( com->M ) {
        ...
        case 42: //M42 -Change pin status via gcode
            if (com->hasP()) {
                ...
                if ((pin_number == FAN_PIN) && (Printer::BoXZY_head == BoXZY_Laser_head)) {
                    // FAN_PIN is sensitive on BoXZY with Laser Head
                    pin_number = -1;
                }
                ...
                // Discard all queued L codes and use the last one pushed.
                uint8_t p = 0;
                while (BoXZYLBuffer.oldest_index != com->L_end_index)
                {
                    p = BoXZYLBuffer.pop();
                }

                Printer::is_L_in_focus_mode = (p > 0);
                set_laser(p);
            }
            break;
...
break;
        ...
        case 899: // TODO: remove when no longer needed
            OUT_P_I_LN("o", BoXZYLBuffer.oldest_index);
            OUT_P_I_LN("u", BoXZYLBuffer.unclaimed_index);
            OUT_P_I_LN("c", BoXZYLBuffer.committed_index);
            OUT_P_I_LN("w", BoXZYLBuffer.write_index);
            break;
...
void Commands::executeGCode(GCode *com) {
    ...
    if(com->hasG()) processGCode(com);
        ...
        // Ignore L-only blocks, they were used to accumulate
        // data in the parsing stage and that data will be
        // consumed by the next G1/G3 executed.
    } else {
