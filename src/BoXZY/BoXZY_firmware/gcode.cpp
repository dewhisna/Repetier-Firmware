...
void GCode::pushCommand()
{
    ...
    {
        cmd->params2 &= ~512; // Ensure !hasL() so later processing doesn't misfire
    }
...
void GCode::executeFString(FSTRINGPARAM(cmd))
{
    ...
    do
    {
        ...
        if(code.parseAscii((char *)buf,false) && ((code.params & 518) || code.hasL()))   // Success
        {
#ifdef DEBUG_PRINT
            debugWaitLoop = 7;
#endif
...
bool GCode::parseBinary(uint8_t *buffer,bool fromSerial)
{
    ...
    if(hasL())
    {
        ...
        params2 &= ~512;  // Disable since we aren't parsing it yet
    }
    if(hasO())
    {
        O = *(float *)p;
