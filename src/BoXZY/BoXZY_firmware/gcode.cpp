...
{
    GCode *cmd = &commandsBuffered[bufferWriteIndex];
    bool has_l = cmd->hasL();

    if (has_l)
    {
        BoXZYLBuffer.committed_index = BoXZYLBuffer.write_index;
        if (Printer::is_L_in_focus_mode)
        {
            Printer::is_L_in_focus_mode = false;
            set_laser(0);
        }

        if (!(cmd->hasG() || cmd->hasM()))
        {
            // *this doesn't use the L data, *this was just pushing it
            // for a future use.
            return;
        }
    }

    // G0 with an L, G1, G2, G3, M42 all consume any pending laser
    // power data.
    //
    // This may suprise folks that, for some crazy
    // reason, do something like:
    // 
    //    L10.0 L11.0 L12.0 ....
    //    G0 X100 L50.0
    //    G1
    //
    // because the G0 will consume, and try to print, all of the
    // queued Ls. TODO: consider using "L*" to pull from the
    // queue, and L# to hardcode one iteration.
    //
    // And:
    // 
    //    L10.0 L11.0 L12.0 ....
    ...
    //    G1
    //
    // would discard queued Ls.
    //
    bool consumes_l = ((BoXZYLBuffer.unclaimed_index != BoXZYLBuffer.committed_index)
        && ( (cmd->hasG() && (has_l || cmd->G == 1 || cmd->G == 2 || cmd->G == 3))
            || (cmd->hasM() && (cmd->M == 42) && has_l)));

    if (consumes_l && (Printer::BoXZY_head == BoXZY_Laser_head))
    {
        cmd->params2 |= 8192; // Set hasL()
        cmd->params |= 4096; // Needs V2 for saving (TODO: implement saving scanline)

        cmd->L_index = BoXZYLBuffer.unclaimed_index;
        cmd->L_end_index = BoXZYLBuffer.committed_index;
        BoXZYLBuffer.unclaimed_index = BoXZYLBuffer.committed_index;
    }
    else if (has_l)
    {
        cmd->params2 &= ~8192; // Ensure !hasL() so later processing doesn't misfire
    }
...
void GCode::executeFString(FSTRINGPARAM(cmd))
{
    ...
    do
    {
        ...
        if(code.parseAscii((char *)buf,false) && ((code.params & 518) || (code.params2 & 8192)))   // Success
        {
#ifdef DEBUG_PRINT
    debugWaitLoop = 7;
#endif
...
bool GCode::parseBinary(uint8_t *buffer,bool fromSerial)
{
    ...
    // TODO: L binary parsing, if/when there's a PC program that can send it.
...
bool GCode::parseAscii(char *line,bool fromSerial)
{
    ...
    BoXZYLBuffer.write_index = BoXZYLBuffer.committed_index;
    ...
    else
    {
        ...
        if((pos = strchr(line,'L'))!=0)
        {
            if (Printer::BoXZY_head == BoXZY_Laser_head)
            {
                // Setting this also tells executeGCode() to ignore it unless it's a G
                // or M too.
                params2 |= 8192; // Set hasL()
                ...
                do {
                    ...
                    float pct = parseFloatValue(&pos);
                    long count = 1;

                    if (*pos == '^')
                    {
                        ++pos;
                        count = parseLongValue(&pos);
                    }

                    while (count > 0)
                    {
                        BoXZYLBuffer.append_pct(pct);
                        --count;
                    }
...
#endif
    if(hasFormatError() || (((params & 518)==0) && !hasL()))   // Must contain G, M, L or T command and parameter need to have variables!
    {
        formatErrors++;
        if(Printer::debugErrors())
...
void GCode::printCommand()
{
    ...
    if(hasL())
    {
        uint16_t i = L_index;
        do {
            Com::printF(Com::tL,BoXZYLBuffer.elts[i]);
            BoXZYLBuffer.inc(&i);
        } while (i != L_end_index);
    }
    if(hasString())
