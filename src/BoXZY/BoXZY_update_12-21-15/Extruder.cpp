...
static BoXZY_head_t new_BoXZY_head;
millis_t new_head_last_change_ms;
...
void Extruder::manageTemperatures()
{
    ...
    for(uint8_t controller=0; controller<NUM_TEMPERATURE_LOOPS; controller++)
    {
        ...
        if (controller == BOXZY_HEAD_TYPE_TEMPERATURE_CONTROLLER_NUMBER)
        {
            BoXZY_head_t h;
            if (act->currentTemperatureC >= BOXZY_HEAD_TYPE_LASER_HEAD_MIN_TEMP_C)
            {
                h = BoXZY_Laser_head;
            }
            else if (act->currentTemperatureC <= BOXZY_HEAD_TYPE_MILLING_HEAD_MAX_TEMP_C)
            {
                h = BoXZY_CnC_head;
            }
            else
            {
                h = BoXZY_3D_Printer_head;
            }
            ...
            {
                new_BoXZY_head = h;
                new_head_last_change_ms = ms;
            }

            if (Printer::BoXZY_head != new_BoXZY_head)
            {
                // Wrap on subtraction overflow desired when ms
                // wraps.
                millis_t elapsed_ms = ms - new_head_last_change_ms;
                if (elapsed_ms < BOXZY_HEAD_TYPE_DEBOUNCE_MS)
                {
                    // Don't try to do temperature control if
                    // the head is being changed or if the connection
                    // is intermittent, which is probably a result of
                    // handling during focusing.
                    continue;
                }
                else
                {
                    Printer::BoXZY_head = new_BoXZY_head;

                    // Turn off heat PWMing for extruders
                    ...
                    // Clear the temp error counter on head change
                    // so that any prior errors don't carry over and
                    // also so that the sensor readings have a little
                    // time to settle in after the noise due
                    // to a head change. This should also allow the user
                    ...
                    {
                        case BoXZY_3D_Printer_head:
                            Com::printFLN(Com::t3DPrinterHeadDetected);
                            break;

                        case BoXZY_Laser_head:
                            Com::printFLN(Com::tLaserHeadDetected);
                            break;
        ...
        {
            // Don't control temperature on non-extruder heads unless in 3D mode.
            // Do control heated bed (or this would be outside the guarding if).
            if (Printer::BoXZY_head != BoXZY_3D_Printer_head)
            {
                continue;
            }
        ...
        else if(act->heatManager == 3)     // 3: dead-time control
        {
            uint8_t output = 0;

            if(act->targetTemperatureC >= 20.0f)
            {
                // Unlike the original Repetier code, we track the slope even
                // when the heater is not enabled so that the values are accurate
                // the moment it turns on.
                float dT_dt = 3.333 * (act->currentTemperatureC - act->tempArray[act->tempPointer]); // 3.333: 1 / 0.300s
                act->tempIState = 0.25 * (3.0 * act->tempIState + dT_dt); // damp raising

                static uint8_t prev_fan_pwm = 0;
                uint8_t fan_pwm = pwm_pos[NUM_EXTRUDER+2];
...
void TemperatureController::updateCurrentTemperature()
{
    ...
    case 98:
    case 99:
        currentTemperature = ((1024<<(2-ANALOG_REDUCE_BITS))-1)-(osAnalogInputValues[sensorPin]>>(ANALOG_REDUCE_BITS)); // Convert to 10 bit result
        break;
    ...
    case 12:
    {
        ...
        short newraw = 0x7FFF; // 0x7FFF: a very high, obviously wrong count 
        short newtemp = 1000;
        currentTemperature = ((1024<<(2-ANALOG_REDUCE_BITS))-1)-currentTemperature;
        while(i<num)
        {
    ...
    case 52:
    {
        ...
        short oldraw = pgm_read_word(&temptable[0]);
        short oldtemp = pgm_read_word(&temptable[1]);
        short newraw = 0x7FFF; // 0x7FFF: a very high, obbiously wrong count 
        short newtemp = 1000;
        while(i<num)
        {
            newraw = pgm_read_word(&temptable[i++]);
